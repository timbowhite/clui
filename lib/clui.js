...
var helpers = {
  ...
  Gauge: function (value, maxValue, width, dangerZone, suffix, noColor) {
    if (maxValue === 0) {
      return '[]';
    } else {
      ...
      var barColor;
      if (noColor) barColor = function(o){return o;};
      else if (value > dangerZone) barColor = clc.red; 
      else barColor = clc.green;

      return '['+
        barColor(Array(barLength).join("|")) +  // The filled portion
        Array(width + 1 - barLength).join("-") +    // The empty portion
      '] ' + (noColor ? suffix : clc.blackBright(suffix));
    }
  },
